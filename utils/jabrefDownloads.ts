 ...
 *
 * The manifest is generated by the update-download-manifest workflow. Resolving
 * the asset by name instead of constructing a versioned filename keeps this
 * static site independent from a server and from packaging-name changes.
 */
...
export async function fetchLatestJabRefRelease(baseURL: string): Promise<JabRefRelease> {
  const response = await fetch(`${baseURL}downloads.json`)

  if (!response.ok) {
    throw new Error(`The local download manifest returned ${response.status}`)
  }

  const release = (await response.json()) as JabRefRelease
  if (!Array.isArray(release.assets)) {
    throw new Error('The local download manifest does not contain assets')
  }
